...
import { Search, Database, CheckCircle2, AlertCircle, ArrowRight, Loader } from 'lucide-react';
...
export function ScrapingMockup() {
    ...
    useEffect(() => {
        ...
        // Simulate finding properties one by one
        let timeouts = [];
        SCRAPED_PROPERTIES.forEach((prop, index) => {
            const timeout = setTimeout(() => {
                setFoundProperties(prev => [...prev, prop]);
            }, (index + 1) * 2000);
            timeouts.push(timeout);
        });

        // Stop scanning visuals after all found
